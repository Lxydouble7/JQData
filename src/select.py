...
file = open('等权组合增持家数r20m40.csv', 'w', newline='', encoding='gbk')
...
for date in time_set:
    ...
    # 选出所有增持的
    increase = mycsv.loc[mycsv['持股变化'] == '增持']
    increase = increase.sort_values('持有基金家数', ascending=False)
    ...
    count = 0
    while count < len(increase):
        # 随机抽样：DataFrame.sample(n=None, frac=None, replace=False, weights=None, random_state=None, axis=None)[source]
        # increase_sample = increase.sample(n=50, replace=False, random_state=None, axis=0)
        # increase_sample = increase_sample.sample(n=20, replace=False, random_state=None, axis=0)
        # 先抽取40家增持比例最大的
        increase_sample = increase.head(40)
        # 再随机从里面抽20家
        #increase_sample = increase
        increase_sample = increase_sample.sample(n=20, replace=False, random_state=None, axis=0)
