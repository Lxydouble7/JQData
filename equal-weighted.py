...
file = open('等权组合.csv', 'w', newline='', encoding='gbk')
...
for date in time_set:
    ...
    while count < len(increase) * 10:
        ...
        col1.append(increase_sample['过去一季收益率'].sum()/30)
        col2.append(increase_sample['相对沪深300指数的收益率1'].sum()/30)
        col3.append(increase_sample['未来一个月'].sum()/30)
        col4.append(increase_sample['相对沪深300指数的收益率2'].sum()/30)
        col5.append(increase_sample['两到三月'].sum()/30)
        col6.append(increase_sample['相对沪深300指数的收益率3'].sum()/30)
        count += 1
